...
1. **Triage order from the corpus report**: (a) ~~And/Or projection adapter
   ...
   `premises-expected-confidence` helper coverage):
   pass=143 close=19 fail=0 unsupported-ir=0 skipped=29 flagged-files=0,
...
11. STV-rule inversion materialization still needs the fold recursion guard
   (see above).
12. Converter gaps: most `!(test (let ...))` forms and non-query test forms
   ...
   distributions.
13. **Frontier bounding for self-feeding rules**: PeTTa's query budget counts
   ...
   accounting.
14. petta facts: bang results print only at process exit (main.pl collects
