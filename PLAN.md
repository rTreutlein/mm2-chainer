...
1. **Triage order from the corpus report**: (a) ~~And/Or projection adapter
   ...
   cyclic guard coverage, and backward helper bookkeeping coverage):
   pass=162 close=19 fail=0 unsupported-ir=0 skipped=10 flagged-files=0,
...
15. **Backward helper bookkeeping**:
   ...
   runtime state.
16. STV-rule inversion materialization still needs the fold recursion guard
   (see above).
17. Converter gaps: most `!(test (let ...))` forms and non-query test forms
   ...
   distributions.
18. **Frontier bounding for self-feeding rules**: PeTTa's query budget counts
   ...
   accounting.
19. petta facts: bang results print only at process exit (main.pl collects
