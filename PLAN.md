...
1. **Triage order from the corpus report**: (a) ~~And/Or projection adapter
   ...
   `rules` shape coverage):
   pass=144 close=19 fail=0 unsupported-ir=0 skipped=28 flagged-files=0,
...
12. STV-rule inversion materialization still needs the fold recursion guard
   (see above).
13. Converter gaps: most `!(test (let ...))` forms and non-query test forms
   ...
   distributions.
14. **Frontier bounding for self-feeding rules**: PeTTa's query budget counts
   ...
   accounting.
15. petta facts: bang results print only at process exit (main.pl collects
